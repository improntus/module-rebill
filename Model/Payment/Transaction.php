...
class Transaction
{
    ...
    private function prepareItems(Quote $quote, Order $order)
    {
        ...
        foreach ($items as $item) {
            ...
            $rebillItem = [
                ...
                'price' => $this->configHelper->limitDecimal($price),
                'quantity' => $itemQty,
                'gateway' => $gateway,
    ...
    private function prepareAdditionalItems(Order $order, Quote $quote, array &$items)
    {
        ...
        $additionalItem = [
            ...
            'price' => $this->configHelper->limitDecimal($total),
            'quantity' => 1,
            'gateway' => $gateway,
            'currency' => $this->configHelper->getCurrency(),
        ];
        if (isset($items[$defaultFrequencyHash]) && count($items) == 1) {
            ...
            foreach ($_items as $hash => $item) {
                ...
                $items[$hash][] = [
                    ...
                    'price_name' => $name,
                    'price' => $this->configHelper->limitDecimal($shipmentPrice),
                    'quantity' => 1,
                    'gateway' => $gateway,
                    'currency' => $this->configHelper->getCurrency(),
                ];
            ...
            if ($hash == $defaultFrequencyHash) {
                foreach ($_items as $item) {
                    $compiledItems[] = [
                        ...
                        'price' => $this->configHelper->limitDecimal($item['price']),
